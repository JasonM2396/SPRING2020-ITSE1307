	...
	do {
		...
		switch (chrMenuChoice) {
			case 's':
				...
				std::cout << "You notice that you are inside an abandoned tunnel, which has sat for there abandoned for decades." << std::endl;
				...
				while (boolWinCondition != true && intNumberOfLives > 0) {
					...
					std::cout << "You have entered a room in the tunnel ..." << std::endl;
					...
					do {
						...
						std::cout << "If you want to search for coins, items or traps, press these keys (c [Coins], i [items], t[traps])? ";
						std::cout << "If you want to quit the game, enter zero (0). " << std::endl;
